...
function Settings() {
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <Card className="p-5 sm:p-6"><div className="flex items-start gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-sky-50 text-sky-700"><SlidersHorizontal size={20} /></span><div><h3 className="text-lg font-extrabold text-slate-900">Operational settings</h3><p className="mt-1 text-sm text-slate-500">Expiry calculations and stock settings use these values across the workspace.</p></div></div><form onSubmit={saveOperational} className="mt-6 space-y-6"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><label className="text-sm font-semibold text-slate-700">Currency<select value={operational.currency} onChange={(event) => setOperational((current) => ({ ...current, currency: event.target.value }))} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 font-normal outline-none focus:border-teal-500"><option value="PKR">PKR</option><option value="USD">USD</option><option value="EUR">EUR</option></select></label><Input id="settings-low-stock" label="Low-stock threshold" type="number" min="0" step="1" value={operational.low_stock_threshold} onChange={(event) => setOperational((current) => ({ ...current, low_stock_threshold: event.target.value }))} /><Input id="settings-expiry-days" label="Expiry warning days" type="number" min="1" max="365" step="1" value={operational.expiry_warning_days} onChange={(event) => setOperational((current) => ({ ...current, expiry_warning_days: event.target.value }))} /><label className="text-sm font-semibold text-slate-700">Timezone<select value={operational.timezone} onChange={(event) => setOperational((current) => ({ ...current, timezone: event.target.value }))} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 font-normal outline-none focus:border-teal-500"><option value="Asia/Karachi">Asia/Karachi</option><option value="UTC">UTC</option><option value="Asia/Dubai">Asia/Dubai</option></select></label></div><div className="flex justify-end"><Button type="submit" loading={savingOperational} leftIcon={<Save size={17} />}>Save operational settings</Button></div></form></Card>

      <Card className="p-5 sm:p-6"><div className="flex items-start gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-amber-50 text-amber-700"><BellRing size={20} /></span><div><h3 className="text-lg font-extrabold text-slate-900">Notification preferences</h3><p className="mt-1 text-sm text-slate-500">These settings are stored now and can be used by future n8n workflows.</p></div></div><form onSubmit={saveOperational} className="mt-6 space-y-3"><Toggle id="expiry-alerts" label="Expiry alerts" hint="Notify when expiry thresholds are reached." checked={operational.expiry_alerts} onChange={(event) => setOperational((current) => ({ ...current, expiry_alerts: event.target.checked }))} /><Toggle id="stock-alerts" label="Low-stock alerts" hint="Notify when inventory needs attention." checked={operational.low_stock_alerts} onChange={(event) => setOperational((current) => ({ ...current, low_stock_alerts: event.target.checked }))} /><Toggle id="daily-summary" label="Daily sales summary" hint="Prepare a daily operational digest." checked={operational.daily_sales_summary} onChange={(event) => setOperational((current) => ({ ...current, daily_sales_summary: event.target.checked }))} /><Toggle id="weekly-summary" label="Weekly business summary" hint="Prepare a weekly performance digest." checked={operational.weekly_business_summary} onChange={(event) => setOperational((current) => ({ ...current, weekly_business_summary: event.target.checked }))} /><Toggle id="critical-alerts" label="Critical alerts" hint="Keep the most urgent operational alerts enabled." checked={operational.critical_alerts} onChange={(event) => setOperational((current) => ({ ...current, critical_alerts: event.target.checked }))} /><div className="flex justify-end pt-2"><Button type="submit" loading={savingOperational} leftIcon={<Save size={17} />}>Save notifications</Button></div></form></Card>
